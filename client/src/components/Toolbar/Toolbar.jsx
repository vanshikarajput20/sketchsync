import { useState, useRef, useEffect } from 'react';
import { useRoomStore } from '../../store/roomStore.js';
import { useUndoRedo }  from '../../hooks/useUndoRedo.js';
import { useSocket }    from '../../hooks/useSocket.js';
import {
  TOOL_PEN, TOOL_ERASER, TOOL_RECT, TOOL_CIRCLE, TOOL_LINE, TOOL_TRIANGLE, TOOL_STAR, TOOL_DIAMOND, TOOL_TEXT,
  BRUSH_SOLID, BRUSH_HIGHLIGHTER, BRUSH_DASHED, BRUSH_DOTTED, BRUSH_CALLIGRAPHY, BRUSH_MARKER, BRUSH_BRUSH, BRUSH_NEON, BRUSH_TEXTURED, BRUSH_GRADIENT
} from '../../lib/operationTypes.js';
import styles from './Toolbar.module.css';

// Tool icons (inline SVG paths for zero dependency)
const TOOL_ICONS = {
  pen:      '✏️',
  eraser:   '⬜',
  rect:     '▭',
  circle:   '○',
  line:     '╱',
  triangle: '▲',
  star:     '★',
  diamond:  '◆',
  text:     'T',
};

const TOOL_LABELS = {
  pen:      'Pen (P)',
  eraser:   'Eraser (E)',
  rect:     'Rectangle (R)',
  circle:   'Circle (C)',
  line:     'Line (L)',
  triangle: 'Triangle (H)',
  star:     'Star (S)',
  diamond:  'Diamond (D)',
  text:     'Text (T)',
};

const PRESET_COLORS = [
  '#1a1a2e', '#ef4444', '#3b82f6', '#22c55e',
  '#f59e0b', '#a855f7', '#06b6d4', '#f97316',
  '#ec4899', '#ffffff',
];

const PRESET_SIZES = [2, 6, 12, 24, 48, 80];
const SHAPES = [TOOL_RECT, TOOL_CIRCLE, TOOL_LINE, TOOL_TRIANGLE, TOOL_STAR, TOOL_DIAMOND];

export function Toolbar() {
  const tool     = useRoomStore((s) => s.tool);
  const setTool  = useRoomStore((s) => s.setTool);
  const roomId   = useRoomStore((s) => s.roomId);
  const userId   = useRoomStore((s) => s.userId);
  const socket   = useSocket();
  const { undo, redo, canUndo, canRedo } = useUndoRedo();

  const [selectedShape, setSelectedShape] = useState(TOOL_RECT);
  const [showShapeDropdown, setShowShapeDropdown] = useState(false);
  const shapeGroupRef = useRef(null);

  useEffect(() => {
    const handleClickOutside = (e) => {
      if (shapeGroupRef.current && !shapeGroupRef.current.contains(e.target)) {
        setShowShapeDropdown(false);
      }
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, []);

  const handleClear = () => {
    if (!window.confirm('Clear the entire board? This cannot be undone.')) return;
    socket.emit('board:clear', { roomId, userId });
  };

  const isShapeActive = SHAPES.includes(tool.activeTool);

  return (
    <div className={styles.toolbar} role="toolbar" aria-label="Drawing tools">
      {/* ── Drawing tools ────────────────────────────────────────── */}
      <div className={styles.group}>
        {/* Pen */}
        <button
          id={`tool-${TOOL_PEN}`}
          className={`${styles.toolBtn} ${tool.activeTool === TOOL_PEN ? styles.active : ''}`}
          onClick={() => setTool({ activeTool: TOOL_PEN })}
          data-tooltip={TOOL_LABELS[TOOL_PEN]}
          aria-label={TOOL_LABELS[TOOL_PEN]}
          aria-pressed={tool.activeTool === TOOL_PEN}
        >
          {TOOL_ICONS[TOOL_PEN]}
        </button>

        {/* Eraser */}
        <button
          id={`tool-${TOOL_ERASER}`}
          className={`${styles.toolBtn} ${tool.activeTool === TOOL_ERASER ? styles.active : ''}`}
          onClick={() => setTool({ activeTool: TOOL_ERASER })}
          data-tooltip={TOOL_LABELS[TOOL_ERASER]}
          aria-label={TOOL_LABELS[TOOL_ERASER]}
          aria-pressed={tool.activeTool === TOOL_ERASER}
        >
          {TOOL_ICONS[TOOL_ERASER]}
        </button>

        {/* Grouped Shapes */}
        <div ref={shapeGroupRef} className={`${styles.shapeGroup} ${isShapeActive ? styles.activeGroup : ''}`}>
          <button
            id="tool-shape"
            className={`${styles.toolBtn} ${isShapeActive ? styles.active : ''}`}
            onClick={() => setTool({ activeTool: selectedShape })}
            data-tooltip={`Shape: ${TOOL_LABELS[selectedShape]}`}
            aria-label={`Shape: ${TOOL_LABELS[selectedShape]}`}
            aria-pressed={isShapeActive}
            style={{ borderTopRightRadius: 0, borderBottomRightRadius: 0, border: 'none' }}
          >
            {TOOL_ICONS[selectedShape]}
          </button>
          <button
            className={styles.dropdownToggle}
            onClick={() => setShowShapeDropdown(!showShapeDropdown)}
            aria-label="Select shape"
            aria-expanded={showShapeDropdown}
          >
            ▾
          </button>
          {showShapeDropdown && (
            <div className={styles.shapeDropdown}>
              {SHAPES.map((s) => (
                <button
                  key={s}
                  className={`${styles.shapeDropdownBtn} ${tool.activeTool === s ? styles.activeShape : ''}`}
                  onClick={() => {
                    setSelectedShape(s);
                    setTool({ activeTool: s });
                    setShowShapeDropdown(false);
                  }}
                >
                  <span style={{ fontSize: '16px' }}>{TOOL_ICONS[s]}</span>
                  <span>{TOOL_LABELS[s].split(' ')[0]}</span>
                </button>
              ))}
            </div>
          )}
        </div>

        {/* Text */}
        <button
          id={`tool-${TOOL_TEXT}`}
          className={`${styles.toolBtn} ${tool.activeTool === TOOL_TEXT ? styles.active : ''}`}
          onClick={() => setTool({ activeTool: TOOL_TEXT })}
          data-tooltip={TOOL_LABELS[TOOL_TEXT]}
          aria-label={TOOL_LABELS[TOOL_TEXT]}
          aria-pressed={tool.activeTool === TOOL_TEXT}
        >
          {TOOL_ICONS[TOOL_TEXT]}
        </button>
      </div>

      <div className={styles.divider} />

      {/* ── Pen Brush Styles (Conditional) ─────────────────────────── */}
      {tool.activeTool === TOOL_PEN && (
        <>
          <div className={styles.group}>
            <label htmlFor="select-brush" className="visually-hidden">Brush Style</label>
            <select
              id="select-brush"
              value={tool.brushStyle}
              onChange={(e) => setTool({ brushStyle: e.target.value })}
              className={styles.brushSelect}
              aria-label="Brush style"
            >
              <option value={BRUSH_SOLID}>Solid Pen</option>
              <option value={BRUSH_HIGHLIGHTER}>Highlighter</option>
              <option value={BRUSH_DASHED}>Dashed Line</option>
              <option value={BRUSH_DOTTED}>Dotted Line</option>
              <option value={BRUSH_CALLIGRAPHY}>Calligraphy</option>
              <option value={BRUSH_MARKER}>Rough Marker</option>
              <option value={BRUSH_BRUSH}>Brush Pen</option>
              <option value={BRUSH_NEON}>Neon Glow</option>
              <option value={BRUSH_TEXTURED}>Chalk Crayon</option>
              <option value={BRUSH_GRADIENT}>Rainbow Gradient</option>
            </select>
          </div>
          <div className={styles.divider} />
        </>
      )}

      {/* ── Color picker ─────────────────────────────────────────── */}
      <div className={styles.group}>
        <div className={styles.colorGrid}>
          {PRESET_COLORS.map((c) => (
            <button
              key={c}
              className={`${styles.colorBtn} ${tool.color === c ? styles.activeColor : ''}`}
              style={{ '--swatch': c }}
              onClick={() => setTool({ color: c })}
              data-tooltip={c}
              aria-label={`Color ${c}`}
              aria-pressed={tool.color === c}
            />
          ))}
        </div>
        {/* Custom color input */}
        <label className={styles.customColorLabel} data-tooltip="Custom color">
          <input
            type="color"
            value={tool.color}
            onChange={(e) => setTool({ color: e.target.value })}
            className={styles.customColorInput}
            aria-label="Custom color picker"
          />
          🎨
        </label>
      </div>

      <div className={styles.divider} />

      {/* ── Stroke width Dots Grid ────────────────────────────────── */}
      <div className={styles.group}>
        <span className={styles.sizeLabel}>Size</span>
        <div className={styles.sizeGrid}>
          {PRESET_SIZES.map((w) => (
            <button
              key={w}
              className={`${styles.sizeBtn} ${tool.lineWidth === w ? styles.activeSize : ''}`}
              onClick={() => setTool({ lineWidth: w })}
              data-tooltip={`${w}px`}
              aria-label={`Size ${w}px`}
              aria-pressed={tool.lineWidth === w}
            >
              <span
                className={styles.sizeDot}
                style={{
                  width: `${Math.max(2, Math.min(18, w / 3 + 1))}px`,
                  height: `${Math.max(2, Math.min(18, w / 3 + 1))}px`,
                }}
              />
            </button>
          ))}
        </div>
      </div>

      <div className={styles.divider} />

      {/* ── Fill toggle (for shapes) ──────────────────────────────── */}
      <div className={styles.group}>
        <button
          className={`${styles.toolBtn} ${tool.filled ? styles.active : ''}`}
          onClick={() => setTool({ filled: !tool.filled })}
          data-tooltip="Toggle shape fill"
          aria-label="Toggle shape fill"
          aria-pressed={tool.filled}
        >
          ▪
        </button>
      </div>

      <div className={styles.divider} />

      {/* ── Actions ──────────────────────────────────────────────── */}
      <div className={styles.group}>
        <button
          id="btn-undo"
          className={styles.actionBtn}
          onClick={undo}
          disabled={!canUndo}
          data-tooltip="Undo (Ctrl+Z)"
          aria-label="Undo"
        >
          ↩
        </button>
        <button
          id="btn-redo"
          className={styles.actionBtn}
          onClick={redo}
          disabled={!canRedo}
          data-tooltip="Redo (Ctrl+Shift+Z)"
          aria-label="Redo"
        >
          ↪
        </button>
        <button
          id="btn-clear"
          className={`${styles.actionBtn} ${styles.danger}`}
          onClick={handleClear}
          data-tooltip="Clear board"
          aria-label="Clear board"
        >
          🗑
        </button>
      </div>
    </div>
  );
}
